...
- `국회의원 발의법률안 (nzmimeepazxkubdpn)` for representative and co-sponsor participation plus official processing results
...
5. Bill proposal volume remains an activity measure. Passage statistics may use `PROC_RESULT`, but must separate original/amended passage from alternative-bill reflection and must exclude proposals without a published result from result-rate denominators.
