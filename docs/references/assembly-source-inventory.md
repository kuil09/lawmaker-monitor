...
| physical meeting attendance           | official plenary (`OO1X9P001017YF13038`) and committee (`OR137O001023MZ19321`) minutes catalogs + National Assembly viewer minutes + `본회의 출결현황 (O4Q5B50011905O18367)` + dated committee-career sheet | same                   | per-member plenary and standing-committee `present`, `absent`, `leave`, and `trip` facts; catalog totals are checked before cursor advancement, and XLSX files fill missing appendices and catalog dates | `exact`         |
...
19. `plenaryMinutesCatalog`
20. `committeeMinutesCatalog`
